...
import static com.aliyun.oss.internal.OSSConstants.*;
...
public class OSSUtils {
    ...
    private static boolean validateBucketName(String bucketName) {
        ...
        return bucketName != null && bucketName.matches(BUCKET_NAMING_REGEX);
    ...
    public static boolean validateObjectKey(String key) {
        ...
        return firstChar != '/' && firstChar != '\\' && (bytes.length > 0 && bytes.length < OBJECT_NAME_MAX_LENGTH);
    ...
    private static String buildCanonicalHost(URI endpoint, String bucket, ClientConfiguration clientConfig) {
        ...
        StringBuilder cannonicalHost = new StringBuilder();
     ...
     * 用JSON格式编码Callback
     */
    private static String jsonizeCallback(Callback callback) {
     ...
     */
    private static String jsonizeCallbackVar(Callback callback) {
        StringBuilder jsonBody = new StringBuilder();
