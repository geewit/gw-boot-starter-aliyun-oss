...
public class OSSDownloadOperation {
    ...
    static class DownloadCheckPoint implements Serializable {
         ...
         * @throws IOException 
         */
        synchronized void update(int index, boolean completed) throws IOException {
         ...
         */
        synchronized boolean isValid(OSSObjectOperation objectOperation) {
            ...
            return this.objectStat.size == meta.getSize() &&
    ...
    static class ObjectStat implements Serializable {
        ...
        static ObjectStat getFileStat(OSSObjectOperation objectOperation,
        ...
        Date lastModified; // 文件最后修改时间
        String digest; // 文件内容摘要，值为ETAG
    }
    ...
    static class DownloadPart implements Serializable {
        ...
        int index; // 分片序号，从0开始编号
        long start; // 分片起始位置
        long end; // 分片片结束位置
        boolean isCompleted; // 该分片下载是否完成
    }
    ...
    private static void createFixedFile(String filePath, long length) throws IOException {
