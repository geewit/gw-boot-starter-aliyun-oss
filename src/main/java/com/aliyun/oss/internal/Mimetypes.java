...
class Mimetypes {
    ...
    synchronized static Mimetypes getInstance() {
        if (mimetypes != null) 
            return mimetypes;
    ...
    private void loadMimetypes(InputStream is) throws IOException {
    ...
    private String getMimetypeByExt(String fileName) {
        ...
        if (lastPeriodIndex > 0 && lastPeriodIndex + 1 < fileName.length()) {
            ...
            if (extensionToMimetypeMap.keySet().contains(ext)) return extensionToMimetypeMap.get(ext);
        }
        return null;
    }
}
